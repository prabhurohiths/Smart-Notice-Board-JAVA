...
public class NoticeServiceImpl implements NoticeService {
	...
	private NoticeDto mapToDto(Notice notice) {
		...
		dto.setExpiryDate(notice.getExpiryDate() != null ? notice.getExpiryDate().toString() : null);
		...
		if (notice.getImagePaths() != null && !notice.getImagePaths().isEmpty()) {
			List<String> fileNames = Arrays.stream(notice.getImagePaths().split(",")).map(String::trim)
	...
	private Notice mapToEntity(NoticeDto dto) {
		...
		if (dto.getExpiryDate() != null) {
			notice.setExpiryDate(LocalDate.parse(dto.getExpiryDate()));
		}
	...
	public NoticeDto createNotice(NoticeDto noticeDto, Long postedById, List<MultipartFile> images) {
		...
		if (noticeDto.getExpiryDate() != null) {
			notice.setExpiryDate(LocalDate.parse(noticeDto.getExpiryDate()));
		}
	...
	public NoticeDto updateNoticeWithImages(Long id, NoticeDto noticeDto, List<MultipartFile> files) {
		...
		if (noticeDto.getExpiryDate() != null) {
			existing.setExpiryDate(LocalDate.parse(noticeDto.getExpiryDate()));
		}
		...
		if (existing.getImagePaths() != null && !existing.getImagePaths().isEmpty()) {
			existingFiles = Arrays.stream(existing.getImagePaths().split(",")).map(String::trim)
					.collect(Collectors.toList());
		}

		List<String> remainingFromFrontend = noticeDto.getImagePaths() != null ? noticeDto.getImagePaths()
				: new ArrayList<>();
	...
	@Override
	public Map<String, Object> filterNotices(String postedBy, Integer year, Integer uploadedYear, String department,
			int page, int size) {
		Pageable pageable = PageRequest.of(page, size, Sort.by("postedDate").descending());

		Page<Notice> noticePage = noticeRepository.filterNoticesFromDb(postedBy, year, uploadedYear, department,
				pageable);
	...
	@Override
	public Map<String, Object> studentFilterNotices(String postedBy, Integer uploadedYear, String department,
			Integer year, int page, int size) {
		Pageable pageable = PageRequest.of(page, size, Sort.by("postedDate").descending());
		Page<Notice> noticePage = noticeRepository.studentFilterNotices(postedBy, uploadedYear, department, year,
				pageable);
	...
	public void deleteNoticeWithRoleCheck(Long id, Long userId) {
		...
		if (isAdmin || (isTeacher && notice.getPostedBy().getId().equals(userId))) {
			if (notice.getImagePaths() != null) {
				Arrays.stream(notice.getImagePaths().split(",")).map(String::trim).forEach(fileName -> {
					try {
